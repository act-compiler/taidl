...
EPHEMERAL_NAME="taidl-rm-${USER_NAME}"
DEFAULT_PERSISTENT_NAME="taidl-default-${USER_NAME}"
...
# Helper: List all persistent containers of form taidl-<base>-<username>
list_persistent_all() {
    docker ps -a --format '{{.Names}}\t{{.Image}}\t{{.Status}}' \
      | awk -v user="${USER_NAME}" -F '\t' '$1 ~ ("^taidl-.*-"user"$") { print $0 }'
}

# Helper: Ensure name is normalized to: taidl-<base>-<username>
normalize_name() {
    local raw="$1"
    local prefix="taidl-"
...
if ! docker image inspect "${IMAGE_NAME}" >/dev/null 2>&1; then
    echo "Image ${IMAGE_NAME} not found locally. Pull it or build from act/docker/."
    exit 1
fi
